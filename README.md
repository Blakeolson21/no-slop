...
The first [measured 32-case campaign](records/campaigns/noslop-evaluation-campaign.md) found 10 expectations, missed 22, and emitted no unmatched findings under both policies. All model-backed reviewer invocations timed out, so the result does not support a superiority claim. Raw captures and latency records are checked in for replay and inspection.
