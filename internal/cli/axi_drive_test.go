...
// production.
func startDriveTestServer(t *testing.T, srv *ipc.Server) (*ipc.Client, string) {
	t.Helper()
...
// gateRoundStateSource serves a run state computed at reconcile time, so a
// test can advance the step's persisted fix-round count between events.
type gateRoundStateSource struct {
	events chan ipc.Event
	run    func() *ipc.RunInfo
}

func (s *gateRoundStateSource) Subscribe(string) (<-chan ipc.Event, func(), error) {
	return s.events, func() {}, nil
}

func (s *gateRoundStateSource) Reconcile(context.Context, string) (*ipc.RunInfo, error) {
	return s.run(), nil
}
...
func TestDriveRun_ConsecutiveFixReviewParksAdvanceByRoundCount(t *testing.T) {
	...
	srv.Handle(ipc.MethodRespond, func(_ context.Context, raw json.RawMessage) (interface{}, error) {
		...
		defer mu.Unlock()
		responds++
		// The fix round completes before the drive loop's next reconciliation
		// and the step re-parks with the same status and findings; only the
		// persisted round count advances.
		fixRounds++
		// Announce the completed round. The event must follow the respond it
		// reports: the reconciler coalesces everything already queued into a
		// single reconciliation, so pre-queued events would collapse into one.
		select {
