...
func newCLISyncFixture(t *testing.T) cliSyncFixture {
	t.Helper()
	nmHome := filepath.Join(t.TempDir(), "ns-home")
...
func TestSyncHelpAndReferenceExposeGuardedModes(t *testing.T) {
	...
	for _, want := range []string{"## no-slop sync", "## no-slop axi sync", "no-slop axi sync --check"} {
...
func TestAxiSyncCheckAndApplyReturnFullStructuredState(t *testing.T) {
	...
	for _, want := range []string{"branch_sync:", "state: behind", "safety: safe_fast_forward", "freshness: live", f.old, f.pushed, "refs/heads/feature/sync", "command: no-slop axi sync"} {
...
func TestAxiSyncEquivalentDivergedCheckAndApply(t *testing.T) {
	...
	for _, want := range []string{"state: diverged", "safety: safe_equivalent_advance", "relation: diverged", "command: no-slop axi sync"} {
		if !strings.Contains(out, want) {
			t.Errorf("check missing %q:\n%s", want, out)
		}
	}
	...
	if got := cliGit(t, f.local, "rev-parse", "refs/no-slop/sync-anchor/"+f.runID); got != f.old {
...
func TestHumanSyncRequiresConfirmationOutsideTTY(t *testing.T) {
	...
	if !strings.Contains(out, "Re-run with `no-slop sync --yes`") {
		t.Fatalf("output:\n%s", out)
	}
	if got := cliGit(t, f.local, "rev-parse", "HEAD"); got != f.old {
		t.Fatal("HEAD changed")
...
// only in the repo's local gate at <NS_HOME>/repos/<id>.git, while the
...
func newCLIRecoverFixture(t *testing.T) cliRecoverFixture {
	...
	cliGit(t, pipeline, "add", "fix.txt")
	cliGit(t, pipeline, "commit", "-m", "no-slop(review): fix")
...
func TestAxiSyncRecoverReturnsCustodyEndToEnd(t *testing.T) {
	...
	for _, want := range []string{"recovered: true", "state: custody_returned", "changed: true", "relation: equal", "no-slop axi run --intent"} {
		if !strings.Contains(out, want) {
...
func TestAxiSyncRecoverDivergedRefusesThenKeepLocalSucceeds(t *testing.T) {
	...
	for _, want := range []string{"safety: blocked_recover_diverged", "refs/no-slop/recover/", "--keep-local"} {
		if !strings.Contains(out, want) {
	...
	if !strings.Contains(out, "preserved_anchor: refs/no-slop/recover/") {
...
func TestHumanSyncRecoverRequiresConfirmationOutsideTTY(t *testing.T) {
	...
	if !strings.Contains(out, "Re-run with `no-slop sync --recover --yes`") {
		t.Fatalf("output:\n%s", out)
	}
	if got := cliGit(t, f.local, "rev-parse", "HEAD"); got != f.submitted {
		t.Fatal("HEAD changed")
