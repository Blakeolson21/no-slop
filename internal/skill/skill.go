...
// Markdown() to the public skills/no-slop/SKILL.md (verified fresh in
...
// installed directory so the agent exposes it as the /no-slop command.
const Name = "no-slop"
const LegacyName = "no-mistakes"
...
const Description = "Validate your code changes through the no-slop pipeline - automated code review, tests, lint, docs, push, PR, and CI - before they reach the configured push target. Use when the user asks to run no-slop, gate or ship or validate their changes, push safely, asks you to do a task and then validate it, or invokes /no-slop."
...
// tools, e.g. `npx skills add kunchenguid/no-slop`) and the copy init
...
// LegacyMarkdown keeps /no-mistakes discoverable as an alias while all
// instructions and commands point at the canonical no-slop identity.
func LegacyMarkdown() string {
	return markdown(LegacyName, "Compatibility alias for the no-slop validation pipeline. Use /no-mistakes for existing automation; new workflows should invoke /no-slop.")
}

func markdown(name, description string) string {
...
validation at Review, and publishes it through Push**; a PR that is merely behind but still clean needs nothing
...
with ` + "`no-slop rerun`" + `. When the failed run has an exact, mechanically
