...
func TestBuildPipelineSummary_AllClean(t *testing.T) {
	...
	if !strings.Contains(md, "[git push no-slop](https://github.com/Blakeolson21/no-slop)") {
...
func TestBuildPipelineSummary_EmitsStructuredStepAttestation(t *testing.T) {
	...
	steps := []*db.StepResult{
		...
		{ID: "lint", StepName: types.StepLint, Status: types.StepStatusParkedForApproval},
	...
	const prefix = pipelineAttestationCommentPrefix
	...
	want := []struct {
		...
		{types.StepLint, types.StepStatusParkedForApproval, ""},
