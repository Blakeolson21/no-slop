...
func TestE2EScriptHonorsExplicitInventoryParent(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("scripts/e2e.sh is a POSIX shell wrapper")
	}
	if _, err := exec.LookPath("bash"); err != nil {
	...
	fakeBin := t.TempDir()
	logPath := filepath.Join(t.TempDir(), "go-env.log")
	parent := filepath.Join(t.TempDir(), "inventories")
	writeExecutable(t, filepath.Join(fakeBin, "go"), `#!/bin/sh
{
  printf 'parent=%s\n' "${NS_E2E_DAEMON_INVENTORY_PARENT:-}"
  printf 'legacy_parent=%s\n' "${NM_E2E_DAEMON_INVENTORY_PARENT:-}"
...
`)
	...
	cmd.Env = append(cmd.Env,
		"PATH="+fakeBin+string(os.PathListSeparator)+os.Getenv("PATH"),
		"NS_E2E_SCRIPT_ENV_LOG="+logPath,
		"NS_E2E_DAEMON_INVENTORY_PARENT="+parent,
	)
	output, err := cmd.CombinedOutput()
	if ctx.Err() == context.DeadlineExceeded {
		t.Fatalf("scripts/e2e.sh timed out\n%s", output)
	...
	text := string(data)
	if !strings.Contains(text, "parent="+parent+"\n") || !strings.Contains(text, "legacy_parent="+parent+"\n") {
		t.Fatalf("go env log = %q, want explicit parent under both aliases", data)
	}
	if strings.Contains(text, "/no-mistakes-e2e-inventories-") || strings.Contains(text, "/no-slop-e2e-inventories-") {
		t.Fatalf("go env log = %q, want no default inventory parent", data)
	}
}
