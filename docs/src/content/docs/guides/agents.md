...
Persistent server agents (Rovo Dev and OpenCode) start a managed HTTP server on first use and reuse it only while the requested working directory and launch environment remain unchanged. A later invocation with a different working directory or environment stops that server and starts a fresh one, so invocation-scoped values such as the [pipeline-owned gate duty](/no-slop/reference/environment/#mo_gate_turn_kind) reach the server process. If a reused server refuses a connection, no-slop also discards it and retries with a fresh server.
