...
When CI is ready - either its registered checks are green or the trusted default-branch config declares [`no_ci: true`](/no-slop/reference/repo-config/#no_ci) with no registered checks - but the PR is still open, `axi run` and synchronous `axi respond` return `outcome: checks-passed` with a help line pointing at the PR instead of waiting for a human merge. An empty check result without that declaration is not ready; see the [CI step reference](/no-slop/reference/pipeline-steps/#ci) for the readiness rules.
