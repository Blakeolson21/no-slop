...
Configuration is optional. Without any config files, `no-slop` defaults to
...
- whether no-slop should infer intent from recent local agent transcripts

Config is split across two files:

| File                         | Scope                         | Full field reference                                          |
| ---------------------------- | ----------------------------- | ------------------------------------------------------------- |
| `~/.no-mistakes/config.yaml` | Global defaults for all repos | [Global Config Reference](/no-slop/reference/global-config/) |
| `<repo>/.no-slop.yaml`   | Per-repo overrides            | [Repo Config Reference](/no-slop/reference/repo-config/)     |

Set `NS_HOME` to relocate the global config directory (the global file becomes `$NS_HOME/config.yaml`).
Bitbucket Cloud credentials come from environment variables rather than config files.
For Azure DevOps, authenticate the `az` CLI with either `az devops login` or `AZURE_DEVOPS_EXT_PAT` for non-interactive daemon auth; see [Environment Variables](/no-slop/reference/environment/).
...
- `agent_path_override`, `agent_args_override`, `acpx_path`, `acp_registry_overrides`, `ci_timeout`, `daemon_connect_timeout`, `step_quiet_warning`, `agent_timeout`, `review_agent_timeout`, `test_agent_timeout`, `log_level`, and `session_reuse` are global-only fields.
...
- no-slop reloads global config while setting up each run, so edits made before starting a run apply to it. For repeatable profiles (for example fast versus deep Codex settings), use separately initialized `NS_HOME` roots; `NS_HOME` moves all no-slop state, not just config.
...
The [Repo Config Reference](/no-slop/reference/repo-config/) owns the exact per-command semantics (including that `commands.test` is targeted, not CI-parity), command process lifetime, and the `ignore_patterns` match rules.
...
Run `no-slop doctor` to check the global runner, and see [Choosing an Agent](/no-slop/guides/agents/) for how agent selection and fallback lists behave.
