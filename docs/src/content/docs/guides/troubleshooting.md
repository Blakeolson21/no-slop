...
First stop for anything: `no-slop doctor`.

## Debug in this order

```mermaid
flowchart TD
  problem["Something is wrong"] --> doctor["Run no-slop doctor"]
...
Symptoms: `no-slop daemon status` shows stopped, or `no-slop` exits with "daemon not running."
...
If the PID file points at a process that's no longer running, you don't need to delete anything by hand. `no-slop daemon stop` and `daemon restart` recognize the dead process, clear the stale PID file and socket, and succeed; `no-slop daemon start` replaces both when it launches the new daemon.

### "a no-slop daemon is already running for this NS_HOME"

This error always means a genuinely live daemon: the lock it reports cannot go stale (see [Daemon & Worktrees](/no-slop/concepts/daemon/) for the singleton-lock model).
Manage that daemon with `no-slop daemon status` and `no-slop daemon stop` instead of deleting the lock file - deleting the file does not release the lock and only weakens the guard.

If the socket exists and the process is running but stuck or unresponsive, `no-slop` bounds the connection wait with [`daemon_connect_timeout`](/no-slop/reference/global-config/#daemon_connect_timeout) and fails fast with an error naming the socket path instead of silently starting a second daemon. Restart the stuck daemon:
...
- **macOS (launchd):** `launchctl list | grep no-slop` and check `~/Library/LaunchAgents/com.kunchenguid.no-slop.daemon.*.plist`
- **Linux (systemd):** `systemctl --user status no-slop-daemon-*` and `journalctl --user -u no-slop-daemon-* -f`
- **Windows (Task Scheduler):** `schtasks /query /tn "no-slop-daemon-*"`

### `NS_HOME` collisions

If you have multiple installs with different `NS_HOME` roots, each gets its own scoped service name (with a short suffix derived from the path). Make sure you're looking at the right one - `no-slop daemon status` reports which.

## `no-slop daemon stop` or `daemon restart` refuses

Symptom: the command refuses because active pipeline runs are in progress and lists them.

`daemon stop` and `daemon restart` both refuse by default while pipeline runs are active and list the affected runs; [Daemon & Worktrees](/no-slop/concepts/daemon/#starting-and-stopping) owns the guard's exact rules, including the `--force` override each command takes and why `--force` does not cover a run that is executing a step. `daemon start` can refuse for that last reason alone, because refreshing a stale service definition restarts the daemon.

`no-slop update` is not a way around a stale install here: self-update is disabled in this build, and the [CLI reference](/no-slop/reference/cli/#no-slop-update) owns the rebuild-from-source path that replaces it.

First inspect each listed run with `no-slop axi status --run <id>`.
A parked CI gate can clear itself after its PR becomes terminal, including after a daemon restart.
The [`ci_timeout` reference](/no-slop/reference/global-config/#ci_timeout) owns the exact fail-closed reconciliation rules, and [Daemon & Worktrees](/no-slop/concepts/daemon/#crash-recovery) owns restart behavior.
...
If the refusal reports a run as `executing` a step, `--force` will not clear it. Wait for that step to finish or park at a gate, or end the run with `no-slop axi abort --run <id>`.
...
`no-slop` will not silently skip review, test evidence, documentation, or agent-assisted lint and report the remaining work as a passed gate.

### Check PATH

The daemon uses the same binary-discovery order described in [Choosing an Agent](/no-slop/guides/agents/). When it's running through a managed service, it reloads `PATH` from your login shell on macOS and Linux and appends common install locations such as `~/.local/bin`, `~/go/bin`, `~/.cargo/bin`, `~/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin`, and `/bin`.
...
A managed daemon started by launchd or systemd inherits only a minimal environment, so it does not see the `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` / `ALL_PROXY` variables from your shell. `no-slop` bakes any proxy variables that are set when you install or refresh the service into the generated service definition. If you set up the proxy after installing, re-run the installer or `no-slop daemon restart` (with the proxy variables exported) so they get baked in, then confirm them in `~/.config/systemd/user/no-slop-daemon-*.service` on Linux or `~/Library/LaunchAgents/com.kunchenguid.no-slop.daemon.*.plist` on macOS. Once baked in, the values survive later restarts and binary upgrades even from a shell that does not export them, so you only need the variables exported the first time. Windows Task Scheduler inherits your logon environment and needs no forwarding.
...
Requested test evidence may still be written under the managed temporary `no-slop-evidence` directory, or under the configured in-repo evidence directory when `test.evidence.store_in_repo` is enabled.
...
`no-slop` refuses the push instead of overwriting that remote work.

Fetch and inspect the configured push target, then rebase or merge the remote work into your branch before pushing through `no-slop` again.
...
`no-slop` pauses with an `ask-user` finding instead of silently bundling that unrelated local work into the PR.
...
## `git push no-slop` doesn't start a pipeline

Symptom: push succeeds but `no-slop` shows no active run.

### Check the remote

```sh
git remote -v | grep no-slop
```

If it's missing, run `no-slop init` again.
Re-running init refreshes an existing gate and repairs the `no-slop` remote when it is missing.
...
For validated registered gates and strictly named legacy gates, `no-slop daemon restart` also installs missing no-slop-managed hooks and refreshes legacy managed hooks. An existing custom pre-receive hook is preserved behind the managed admission wrapper.
...
If `notify-push.log` mentions `invalid gate path: .`, refresh the managed hook with `no-slop init` or `no-slop daemon restart`, then push again.
...
If the gate is older, re-running `no-slop init` or restarting the daemon also reapplies hook-path isolation when Git supports `config --worktree`.
That protects the gate hook if a tool such as Husky wrote `core.hookspath` into shared git config from inside a linked worktree. [Crash recovery](/no-slop/concepts/daemon/#crash-recovery) owns the gate validation and migration rules used during restart.

## PR step is skipped

Symptom: pipeline completes but the PR step shows `skipped`.

Check the [Provider Integration](/no-slop/guides/provider-integration/) requirements. Most common causes:
...
Once the CI monitor reports readiness and the PR is mergeable, the CI panel shows `✓ Checks passed` and the terminal title switches to `Checks passed`, so you can tell when to go merge the PR; the signal clears automatically if checks start re-running or a new failure appears. A trusted [`no_ci: true` declaration](/no-slop/reference/repo-config/#no_ci) can establish readiness for a zero-check repository; an empty forge response without that declaration is not ready. The [CI step reference](/no-slop/reference/pipeline-steps/#ci) owns the exact readiness and signal-clearing rules, including GitHub Actions runs that do not appear in the PR check rollup.

How long the monitor runs is controlled by `ci_timeout` in `~/.no-mistakes/config.yaml`, an idle timeout that re-arms whenever the upstream default branch advances; the [`ci_timeout` field reference](/no-slop/reference/global-config/#ci_timeout) owns the default, the `unlimited` keyword and its aliases, and the exact re-arm semantics.
...
You can approve, fix, or skip from the TUI or `no-slop axi respond`.
Use `no-slop axi abort` only when you mean to cancel the whole active run.

## Step looks quiet or wedged

Symptom: `no-slop axi status` shows an active step with `last_activity` prefixed by `quiet`, or a review/test/lint step appears to run for longer than expected.

`quiet` means the step has not recorded a step-log line or native-agent lifecycle event for longer than [`step_quiet_warning`](/no-slop/reference/global-config/#step_quiet_warning).
...
```sh
no-slop axi status
no-slop axi logs --step <step> --full
```
...
If the step is parked at a gate, use `no-slop axi respond` instead of waiting.
If the run is genuinely stuck and you want to discard it, use `no-slop axi abort`.
Start a new run only after abort confirms the terminal state; see the [abort command contract](/no-slop/reference/cli/#no-slop-axi-abort).
...
This keeps your gate repos, database, and config but clears transient state. For a full wipe, see the [Uninstall section](/no-slop/start-here/installation/#uninstall).
...
- File an issue: <https://github.com/Blakeolson21/no-slop/issues>
