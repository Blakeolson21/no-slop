...
- Resumes a clean active run interrupted after its validation checkpoint only when its delivery state is mechanically reconstructable; the [pipeline-step reference](/no-slop/reference/pipeline-steps/#reusing-validation-after-a-delivery-failure) owns eligibility, the in-progress CI-monitor exclusion, and every invalidation rule
