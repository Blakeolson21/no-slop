...
- Preserves a run interrupted while actively monitoring CI for an already-created PR as terminal `ci_monitor_interrupted` instead of a pipeline failure; the PR remains open and visible
...
- Removes orphaned worktree directories via `git worktree remove --force` - but never one whose run is still `pending` or `running`; an interrupted-CI worktree is also preserved when its head differs from the last pushed head, because it may contain an unpushed CI repair
