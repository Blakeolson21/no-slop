---
title: Introduction
description: What no-slop is and why it exists.
---

`no-slop` puts a local git proxy in front of your real remote. Push to
`no-slop` instead of `origin`, and it spins up a disposable worktree, runs
...
`no-slop` sits in that gap. It gives you a deliberate local gate before the
...
`no-slop` is a separate remote on purpose.

- `origin` is never rewritten or hijacked.
- `git push origin` still behaves exactly like normal Git.
- `git push no-slop` is an explicit signal that this branch should go
...
```mermaid
flowchart LR
  repo["Your repo"] -->|"git push no-slop"| gate["Local gate repo"]
...
into the gate by pushing to the `no-slop` remote.
...
- A `/no-slop` agent skill so a coding agent can do a task and gate it, or gate existing committed work, backed by a non-interactive `no-slop axi` interface.
- A setup wizard when you run bare `no-slop` with no active run on the current branch - it walks you through creating a branch, committing, and pushing through the gate, then attaches if the daemon registers the new run.
...
- **`git push no-slop`** - the explicit Git path. You push a committed branch to the gate remote instead of `origin`, and the daemon takes it from there. See [Quick Start](/no-slop/start-here/quick-start/).
- **`no-slop`** - the terminal UI. Run it after making changes and a [setup wizard](/no-slop/guides/setup-wizard/) walks you through branch, commit, and push, then attaches to the live run so you can watch, approve, fix, skip, or abort each step.
- **`/no-slop`** - the agent skill. Tell a coding agent `/no-slop <task>` to have it do the task, commit it on a feature branch, and then gate it with that task as intent; use bare `/no-slop` to gate existing committed work. It resolves safe findings on its own and stops to relay anything that needs your decision. See [Driving no-slop as an agent](/no-slop/guides/agents/#driving-no-slop-as-an-agent).

`no-slop init` installs the `/no-slop` skill at user level for every supported agent, so it works in all your repos. The skill drives `no-slop axi`, a non-interactive command surface that prints [TOON](https://toonformat.dev) to stdout, so an agent reaches the same gate and the same approval points you get in the TUI.

## Next

- [Quick Start](/no-slop/start-here/quick-start/) - first push in five minutes
- [Installation](/no-slop/start-here/installation/) - full install options
- [The Gate Model](/no-slop/concepts/gate-model/) - architecture and design decisions
