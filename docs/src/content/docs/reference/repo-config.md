...
**And a reviewed pass clears only the lenses its own change could have exercised.** Each record names the kinds of content the run judged, some subset of source, tests, and documentation, and that list is derived from the changed paths rather than taken from `--change-class`, because a self-asserted field decides nothing here. A lens whose question is about tests, such as `test-capitulation` or `self-consistent-oracle`, is cleared only by a pass over a change that contained tests, and a lens whose question is about code, such as `fail-open-default`, only by a pass over a change that contained code. `scope-expansion` and `asserted-followup-without-artifact` are cleared by any content, because a documentation change really can exceed its stated scope or claim a follow-up that does not exist. Without this, a documentation change that passed a real two-round review cleared an accumulated test escalation while containing no tests at all, which is not theoretical in a repository where editing an instruction file reaches `full-adversarial` on the axes alone. A record that does not say what it judged, including one written before this field existed, clears nothing.

**A run that never reached a verdict is not history.** A record is appended whenever a decision was reached, including with outcome `error` when the run refused, which is correct for the audit trail and is not evidence about the lane. Only a record whose outcome is `pass` or `fail` makes a lane one the gate has judged before, so shedding a first-contact escalation cannot be bought with one throwaway invocation. Refused records stay in the retained window and still carry their findings.
...
**A change scores as mechanical only when its tokens actually moved.** An identifier substitution scores the cheapest novelty when the judged file itself stopped declaring the old name and started declaring the new one in the same edit, the file is a consistent substitution throughout, the old name appears nowhere else in the change set, and no high-risk path is in the diff. A change whose token stream did not move at all is not a substitution and scores as changed logic instead, because the previous route fell through to a comparison that was true of every change ever gated and printed a substitution claim over a diff containing none. In Python, shell, YAML, make, Haskell, Nim, F#, and every extension the classifier does not recognize, a rename must also leave the line structure unchanged, because where a line breaks is syntax there: moving one `return` one level in turns an all-checks-must-pass loop into first-check-wins with a byte-identical token stream. The accepted cost is that a pure reformat in one of those languages buys a review round. A rename campaign spanning several files also scores as changed logic; the alternative was letting one throwaway file vouch for a guard swap in a file it never referenced.
...
- Read the exit code, not the log: 0 is a clean run, 1 is a blocking finding, and 2 is the gate refusing to validate at all, which includes a refused tier request, a `--base` argument, an unusable base, and an unavailable reviewer. A command-line run is advisory, so 0 and 1 close with `advisory-clean` and `advisory-blocked` rather than a verdict; only an orchestrator that supplied `slopcli.Options.PipelineBase` gets `verdict: pass` or `verdict: fail`.
