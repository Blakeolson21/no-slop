...
The NoSlop corpus stores recorded diffs with independent expected findings. The current public campaign has 32 synthetic cases covering every reviewer lens plus deterministic leak and stale outbound-prose checks. Each directory under `corpus/seeds/` contains:
