...
- Bounds its agent turns with [`review_agent_timeout`](/no-slop/reference/global-config/#review_agent_timeout): a round's optional fix turn and its rereview turn share one budget, each later auto-fix round starts a fresh one, and an expired budget cancels the agent and fails the step rather than leaving the run active indefinitely
