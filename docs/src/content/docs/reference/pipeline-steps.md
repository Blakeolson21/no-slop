...
- Immediately before publication, the complete title and body are scrubbed of current-user and conventional Linux, macOS, and Windows home-directory prefixes. This final boundary covers agent prose, intent, findings, summaries, captured output, and artifact paths without relying on every producer to redact independently.
