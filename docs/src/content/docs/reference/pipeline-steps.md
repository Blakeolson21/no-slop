...
- On GitHub, if the exact head tree contains no workflow whose `pull_request` or branch-filtered `push` trigger can run for this PR, parks as ask-user and explicitly reports that nothing tested the commit. Manual, scheduled, tag-only, and nonmatching push workflows do not create an impossible wait; unreadable workflow definitions keep waiting fail-closed
