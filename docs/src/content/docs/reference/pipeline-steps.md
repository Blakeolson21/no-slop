...
- If GitHub refuses the primary rollup because the credential lacks that capability, falls back to exact-head latest-attempt Actions jobs. The fallback can certify green only when the base branch's required checks are readable, every context maps exactly once, and app-bound identities require the GitHub Actions app; missing, ambiguous, incomplete, wrong-head, or wrong-app evidence stays unavailable rather than green
