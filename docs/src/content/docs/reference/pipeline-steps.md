...
This is the per-step reference. For the overview and rationale, see [Pipeline](/no-slop/concepts/pipeline/). For the fix loop, see [Auto-Fix Loop](/no-slop/concepts/auto-fix/).
...
The steering still allows requested test evidence under the managed temporary `no-slop-evidence` directory or the configured in-repo evidence directory, plus incidental temp or cache writes from normal development tools.
Configured shell commands and one-shot agent subprocesses are scoped to their step: when the invocation exits, fails, or is cancelled, no-slop terminates remaining child processes it spawned so background workers do not outlive the run.
When configured Test or Lint command output exceeds 64 KiB, the complete output remains in the authoritative step log while findings, IPC responses, and repair prompts receive a valid-UTF-8 head-and-tail projection capped at 64 KiB. The truncation marker reports the exact original and omitted byte counts and points to `no-slop axi logs --step <step> --full` for the complete output.
Commits created by the shared Review, Test, Document, and Lint fix path, plus CI repair commits, use the configurable [`commit.fix_message`](/no-slop/reference/global-config/#commitfix_message) template.
...
- With GitHub fork routing, the pushed-branch target is the fork branch fetched into `refs/remotes/no-slop-push/<branch>`
...
- Includes user intent when the run has supplied intent or transcript matching found a relevant local agent session; the detailed provenance semantics are documented in [Intent extraction](/no-slop/guides/agents/#intent-extraction)
...
- Before Review, Test, Document, Lint, or CI adopts a post-review commit, persists the unresolved review gate truth and its uncertified source-run range; persistence failure refuses the head adoption. The next run's initial review restores that truth and receives the same pipeline-authored provenance framing so the replacement reviewer is not cold. A recovered selection whose fixer produced no delta binds a review-only round that re-parks the selected findings instead of approving a head no fix addressed, and an ignored-only fix delta keeps that recovered selection at the gate rather than silently clearing it. A later rebase remaps the persisted SHAs onto the rewritten head. The range is cleared only after a completed review whose approved head equals or descends from the range tip; parked, failed, skipped, and aborted reviews leave it in place
...
- Atomically records the exact commit examined when a full review completes successfully; a parked review retains its candidate only for recovery, while failed, skipped, superseded, and legacy reviews grant no inferred approval authority. Promoting any new run head - a fixer commit, a late CI repair, or terminal head reconciliation - atomically revokes that authority and clears the review's findings while preserving the review truth in the branch's uncertified range, so the repaired head must pass review again
- Retains the commit each recorded review round examined in the local bare gate as `refs/gate-evidence/<run-id>/<round>`, so the reviewed tree stays inspectable after the disposable worktree is gone. A round that recorded no commit pins nothing; re-pinning the same commit is a no-op; a round that would repoint an existing ref at a different commit fails the step rather than silently changing what the evidence claims was reviewed. These refs exist only in the gate, never in your working repo, and no-slop neither expires nor deletes them - `init` and daemon startup configure the gate so garbage collection never reclaims that namespace's reflogs, and the refs go away only when the gate itself does

**Approval:** required if any finding meets the repository's [`blocking_severity`](/no-slop/reference/repo-config/#blocking_severity) floor - by default `error` or `warning`. Findings with `action: ask-user` pause for approval instead of entering the normal auto-fix loop. This is for findings that challenge the author's intent, not routine correctness, reliability, or security fixes that may need to re-add a small amount of deleted logic. With the default `auto_fix.review: 0`, blocking review findings park for approval even when their action is `auto-fix`; setting repo or global `auto_fix.review` above `0` re-enables the automatic review fix loop for eligible `auto-fix` findings. Findings with `action: no-op` are informational only. The shared [finding-action model](/no-slop/concepts/auto-fix/#finding-actions) owns the behavior for a missing `action`.
...
At entry to every remaining step in the fixed pipeline order - Test, Document, Lint, Push, PR, and CI - no-slop compares the live worktree `HEAD` with the pipeline-recorded head. An equal head or a pipeline-descendant commit continues. A backward reset, divergent sibling, or unverifiable relationship fails the run before that step performs work, including for steps that would not create a commit.
...
[`commands.test`](/no-slop/reference/repo-config/#commandstest) owns the configuration contract for any explicit baseline command.
...
- Once a configured `commands.test` baseline returns, that measured result is authoritative. If it passed and the later evidence agent becomes unavailable, the step completes with a non-blocking `evidence-unavailable` warning and records `testing_summary` as `unavailable: <reason>` instead of rewriting the suite result as failed. When `commands.test` is empty, the agent is the measurement, so an unavailable agent still fails the step.
...
Combined-pass lint findings use the same gate: findings that meet the repository's [`blocking_severity`](/no-slop/reference/repo-config/#blocking_severity) floor pause for a decision, while `info` findings do not.
...
- Commits any uncommitted agent changes with message `no-slop: apply agent fixes`
...
- Bitbucket Cloud credentials are missing (`NS_BITBUCKET_EMAIL` or `NS_BITBUCKET_API_TOKEN`)
...
- While the PR stays open, the TUI and terminal title show `Checks passed` once CI readiness is established and known mergeability is clear, and `no-slop axi` returns `outcome: checks-passed` with successful-output reporting instructions so agents can summarize the run, ask the user to review and merge, and list any pipeline fixes instead of waiting
- An empty forge check list is never treated as green unless the trusted default-branch config declares [`no_ci: true`](/no-slop/reference/repo-config/#no_ci). That declaration is positive durable evidence the repository intentionally has no CI; absence means CI is expected and delayed registration stays not-ready. If checks still appear on a declared no-CI repo, their actual states are honored
- If the default branch moves after `checks-passed`, keeps watching the same PR; a clean behind PR needs no action, while an actual GitHub, GitLab, or Azure DevOps merge conflict is auto-fixed by rebasing onto the base, restarting validation at Review, and publishing through the Push step's force-push safety guard
...
- Once every check has finished, classifies each terminally failed check by the provider's own reported outcome before anything escalates; [`ci.rerun_transient`](/no-slop/reference/repo-config/#cirerun_transient) owns which outcomes count as the provider reporting itself
