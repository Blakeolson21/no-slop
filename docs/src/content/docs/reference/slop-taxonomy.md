...
No matching history preserves the unconditioned v1 route and prints that default. Unreadable or malformed history selects `full-adversarial` because the policy could not establish that a lighter tier is safe. A lower `--tier` that contradicts any policy-driven escalation is refused unless the operator also supplies `--force-tier`; the output prints both the escalation and any forced override.
