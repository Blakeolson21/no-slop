...
When the blocklist file is tracked at the base ref, its content is read from there and a differing head copy is reported as drift; when it is not tracked there, the worktree copy is read and the run says on its own state line that this content sits outside the base-ref boundary. `docs/src/content/docs/reference/repo-config.md` owns that boundary.

A file carrying Unicode format or control characters is scanned twice, as written and with those characters removed, and the mandatory-check line names it as read more than one way. One zero-width space inside a credential was enough to break the pattern while leaving the file text and the check line reading `completed (0 findings)`.
...
Every completed run reports the status and finding count for the lens pre-check, leak scan, test-count floor, and prose oracle. A check that could not look reports `not armed`, a check that looked at less than everything reports `reduced coverage`, and a check that had to read the same bytes more than once to look at all of it reports `read more than one way`, so a silent zero is never read as a clean result. The three are distinct claims: the last one gained coverage rather than losing it. A disabled test-count floor is stated explicitly. The other lens pre-checks run at every tier.
