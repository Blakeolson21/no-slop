...
Creates or refreshes a local bare repo, installs the managed pre-receive admission and post-receive notification hooks, best-effort isolates the gate repo's hook path from shared git config changes when Git supports `config --worktree`, adds or repairs the `no-slop` git remote plus the `no-mistakes` compatibility remote, records the default branch resolved by the origin preflight below, records or updates the repo in SQLite, installs the `/no-slop` agent skill at user level into `~/.claude/skills/no-slop/SKILL.md` and `~/.agents/skills/no-slop/SKILL.md`, and ensures the daemon is running, installing the managed service when available and falling back to a detached daemon otherwise.

Before any of that, `init` asks `origin` for its live ref advertisement, resolves the default branch `origin` advertises for `HEAD` (falling back to `main` when `origin` advertises no usable `HEAD` symref), and requires that `refs/heads/<default_branch>` actually exists at `origin`. A local branch or a remote-tracking ref of the same name is deliberately not accepted as evidence, because it says nothing about what `origin` has. Both failure modes stop before a gate remote is provisioned or any database row is written, and stay distinct:

- **Reachable but absent.** The error names the repository path, the `origin` URL, the exact expected ref, and the branch refs `origin` advertised instead (at most eight, plus a count of the rest so the diagnostic itself stays bounded).
- **Unreachable.** `init` fails rather than skipping the check, with a different message naming the transport reason, so you repair connectivity or authentication instead of creating a branch that was never missing.

Credentials embedded in an `origin` URL are redacted in every one of these messages. This preflight runs on a refresh of an already-initialized repo too, not only on a fresh `init`.
...
For retryable rulings, supply `--run <id> --step <name> --idempotency-key <key>`. Keys are scoped to the run, are at most 128 bytes, and contain only ASCII letters, digits, dots, underscores, and hyphens. Repeating the identical ruling with the same key reuses its original receipt without funding another round, including after the run advances, ends, or the daemon restarts. By default the retry resumes synchronous observation from authoritative run state; `--no-wait` is the only mode that returns the receipt immediately. Reusing the key for a different step or ruling payload fails. Use a new key only for a distinct ruling.

Every submitted response prints its supplied or generated key before sending and reports the run, step, and answered round upon acceptance. After an uncertain transport error, use `no-slop axi respond --receipt --run <id> --idempotency-key <key>` from any directory to read the existing database without creating application directories or a database, migrating schema, parsing configuration, starting the daemon, or sending a ruling. SQLite may create its normal WAL/SHM coordination sidecars while opening the existing database read-only. If no receipt exists yet, the original request may still be in flight; retry only the identical ruling with its original key. Old IPC callers without a key do not receive retry protection, and old daemons must be upgraded to support receipts. The [environment reference](/no-slop/reference/environment/#what-stays-local-and-what-leaves-the-machine) owns what telemetry this lookup emits.
...
The same successful-output reporting instructions apply when synchronous `axi respond` reaches an outcome.
...
Removes the `no-slop` remote and the `no-mistakes` compatibility remote, deletes the bare repo directory, cleans up worktrees, and deletes the database record (cascades to runs and steps).
