...
Gate summaries are bounded in this default status view; truncated summaries disclose their original length, and the gate help points to `no-slop axi logs --step <step> --full` for the complete step log. Finding descriptions render in full so an agent can relay ask-user decision content verbatim.
