...
Every submitted response prints its supplied or generated key before sending and reports the run, step, and answered round upon acceptance. After an uncertain transport error, use `no-slop axi respond --receipt --run <id> --idempotency-key <key>` to read the existing database without creating application directories or a database, migrating schema, parsing configuration, starting the daemon, emitting telemetry, or sending a ruling. SQLite may create its normal WAL/SHM coordination sidecars while opening the existing database read-only. If no receipt exists yet, the original request may still be in flight; retry only the identical ruling with its original key. Old IPC callers without a key do not receive retry protection, and old daemons must be upgraded to support receipts.
