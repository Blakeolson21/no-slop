<h1 align="center"><code>git push no-slop</code></h1>
<p align="center">
...
`no-slop` 在你真实的远端前面放了一个本地 git 代理。
把分支推给 `no-slop` 而不是 `origin`，它会拉起一个用完即弃的 worktree，跑一条 AI 驱动的校验流水线，**只有每一项检查都通过后**才把分支转发到配置的推送目标，并自动开出一个干净的 PR。
...
- **agent 原生** -- `/no-slop` 既能让编码 agent 完成一个任务再过网关，也能直接为已提交的工作过网关：它跑完流水线、让流水线应用安全的修复，剩下的升级给你。
- **人始终说了算** -- 自动修复，还是逐条审查 findings，你决定。
- **默认就是干净 PR** -- 推送、开 PR、盯 CI、自动修复失败，一气呵成。

完整文档：<https://kunchenguid.github.io/no-slop/>
...
安装脚本从 `Blakeolson21/no-slop` 下载当前构建。`no-mistakes` 命令在兼容期内继续指向同一二进制文件，物理状态目录也暂时保留为 `~/.no-mistakes`。如需从源码更新，请重新构建并原子替换已安装的二进制文件：
...
安装目录由 `NS_INSTALL_DIR` 决定，默认为 `~/.no-mistakes/bin`；完整说明见本仓库的 `docs/src/content/docs/start-here/installation.md`。

Windows、Go install 以及从源码构建的说明，见[安装指南](https://kunchenguid.github.io/no-slop/start-here/installation/)。
...
如果是 GitHub fork 贡献，让 `origin` 指向父仓库，并用 `no-slop init --fork-url <your-fork-url>` 初始化。
...
- **`git push no-slop`** -- 显式的 Git 路径。把已提交的分支推给网关 remote，而不是 `origin`。
- **`no-slop`** -- TUI。改完之后运行它（无需先提交），向导会带你建分支、提交、推过网关，然后挂到这次运行上。`no-slop -y` 会把这一切自动做完。
- **`/no-slop`** -- agent skill。用 `/no-slop <task>` 让编码 agent 完成一个任务再过网关，或用裸 `/no-slop` 为已提交的工作过网关。它跑完流水线、让流水线应用安全的修复，并在任何需要人来拍板的地方停下来问你。

`no-slop init` 会为 Claude Code 及其他 agent 安装 `/no-slop` skill。底层上这个 skill 驱动的是 `no-slop axi` -- 同一套审批流程的非交互式 TOON 接口。

完整的首次运行走查见[快速上手](https://kunchenguid.github.io/no-slop/start-here/quick-start/)。

## 开发

```sh
make build   # 构建 bin/no-slop（带版本信息）
...
make skill   # 重新生成已提交的 no-slop skill 文件
make fmt     # 运行 gofmt -w .
